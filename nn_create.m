...
for k = 1 : nn.depth-1
    ...
    if strcmp(nn.optimization_method,'Momentum')|| strcmp(nn.optimization_method,'RMSPropwNM')
        nn.vW{k} = zeros(height,width);
        nn.vb{k} = zeros(height,1);
    end; 
    if strcmp(nn.optimization_method,'AdaGrad')  ||strcmp(nn.optimization_method,'RMSProp') || strcmp(nn.optimization_method,'Adam')|| strcmp(nn.optimization_method,'RMSPropwNM')
    ...
    if strcmp(nn.optimization_method,'RMSProp')
            nn.Wtild{k}=zeros(height,width);
            nn.btild{k}=zeros(height,1);
    end
