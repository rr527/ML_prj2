...
for k = 1 : nn.depth-1
    if nn.batch_normalization == 0
        ...
        elseif strcmp(nn.optimization_method, 'RMSPropwNM')
            rho=0.9; alpha=0.1;
            nn.rW{k}=rho*nn.rW{k}+(1-rho)*nn.W_grad{k}.^2;
            nn.rb{k}=rho*nn.rb{k}+(1-rho)*nn.b_grad{k}.^2;
            nn.vW{k}=alpha*nn.vW{k}-nn.learning_rate*nn.W_grad{k}./sqrt(nn.rW{k}+0.00001);
            nn.vb{k}=alpha*nn.vb{k}-nn.learning_rate*nn.b_grad{k}./sqrt(nn.rb{k}+0.00001);
            nn.W{k}=nn.Wtild{k}+nn.vW{k};
            nn.b{k}=nn.btild{k}+nn.vb{k};
        end;
    else
        if strcmp(nn.optimization_method, 'normal')
            nn.W{k} = nn.W{k} - nn.learning_rate*nn.W_grad{k};
